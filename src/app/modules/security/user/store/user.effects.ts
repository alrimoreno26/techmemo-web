...
export class UserEffects {
    ...
    loadUserBasic$ = createEffect(() =>
        this.actions$.pipe(
            ofType(fromUserActions.loadUserBasic),
            switchMap(({lazy}) =>
                this.userAdminServices.loadBasic(lazy).pipe(
                    map((data) => fromUserActions.loadUserBasicSuccess({data})),
                    catchError(error => of(fromUserActions.userFailRequest({error})))
                )
            )
        )
