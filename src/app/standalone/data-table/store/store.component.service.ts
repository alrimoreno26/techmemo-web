...
import {AbstractService} from '../../../core/services/abstract.services';
...
export class StoreComponentService<T> extends ComponentStore<EntityState<T> | any> {
  ...
  hasUpdatePath = true;
  dataKey = 'id';
  readonly setAll = this.updater((state, entities: T[]) => ({...state, entities, total: entities.length, loaded: true}));
  ...
  readonly setUpdate = this.updater((state, update: T | any) => {
    ...
    return {
      ...state,
      entities: state.entities.map((m: { [x: string]: any; }) => m[this.dataKey] === data[this.dataKey] ? {...m, ...data} : m),
  ...
  readonly setSelectById = this.updater((state, id: string | number) => ({...state, selected: state.entities.find((f: T | any) => f[this.dataKey] === id)}));
  /**
   * Reference of dialog
